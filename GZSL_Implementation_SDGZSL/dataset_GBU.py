...
class DATA_LOADER(object):
    def __init__(self):
        ...
        if dataset in ['FLO_EPGN','CUB_STC']:
            if self.finetune:
                self.read_fine_tune()
            else:
                self.read()
        elif dataset in ['CUB', 'AWA2', 'APY', 'FLO', 'SUN']:
    ...
    def read_fine_tune(self):
        ...
        matcontent = sio.loadmat(dataroot + "/" + dataset + "/" + image_embedding + "_finetuned.mat")

        feature = matcontent['features'].T
        label = matcontent['labels'].astype(int).squeeze() - 1
        # feature = matcontent['features'].T
        matcontent = sio.loadmat(dataroot + "/" + dataset + "/data.mat")
        # feature = matcontent['features'].T
        # label = matcontent['labels'].astype(int).squeeze() - 1
    ...
    def read(self, opt):
        ...
        matcontent = sio.loadmat(dataroot + "/" + dataset + "/label.mat")
...
class FeatDataLayer(object):
    def __init__(self, label, feat_data):
