...
class DAZLE(nn.Module):  
    ...
    # r: number of region 
    # k: number of classes  
    # i: number of attribute 
    # h : hidden attention dim  
    #####
    def __init__(self,dim_f,dim_v,  
        ...
        self.dim_f = dim_f  # dimension of feature from ResNet101 (2048)
        self.dim_v = dim_v  # dimension of attribute features from w2v (300)
        self.dim_att = att.shape[1]  # dimension of each semantic attribute (85)
        self.nclass = att.shape[0]  # number of classes or categories (50)
        self.hidden = self.dim_att//2 # number of hidden neurons are half of the dimension of semantic attributes (85//2)
        self.init_w2v_att = init_w2v_att # initial w2v attribute features are the output of w2v already calculated beforehand
        self.non_linear_act = non_linear_act
        self.loss_type = loss_type # Cross-entropy (CE loss function)
        if is_conv:#if is_conv is true, the feature dimension will reduce from 2048 to 1024
            r_dim = dim_f//2
        ...
        if init_w2v_att is None: #if there is no initial attribute feature then initialize it to zero and update it during network training
            self.V = nn.Parameter(nn.init.normal_(torch.empty(self.dim_att,self.dim_v)),requires_grad = True)  
        else:# otherwise, normalize it and clone it as a parameter, trainable_w2v is true means we will update it during network training
        ...
        # semantic attributes are fixed, no need to update them
        self.att = nn.Parameter(F.normalize(torch.tensor(att)),requires_grad = False)     
        
        # initialize weights with normal distribution, make it trainable 
        ...
        # initialize weights with zero, make it trainable
        ...
        ## second layer attenion conditioned on image features, initialize with zero, make it trainable
        ...
        self.P = torch.mm(self.att,torch.transpose(self.att,1,0)) # find correlation att.att transpose, size of P is 50, 50 
        assert self.P.size(1)==self.P.size(0) and self.P.size(0)==self.nclass  
        self.weight_ce = nn.Parameter(torch.eye(self.nclass).float(),requires_grad=False)# 50, 50 identity matrix
        #nn.Parameter(torch.tensor(weight_ce).float(),requires_grad = False)  
        ...
        self.seenclass = seenclass  # indexes of seen class images
        self.unseenclass = unseenclass  # indexes of unseen class images
        self.normalize_att = normalize_att   
        
        if is_bias:# if bias is true
        ...
        if desired_mass == -1:  
            ...
            #nn.Parameter(torch.tensor(self.unseenclass.size(0)/self.nclass),requires_grad = False)
            #nn.Parameter(torch.tensor(0.1),requires_grad = False)#  
        else:  
            self.desired_mass = desired_mass
            #nn.Parameter(torch.tensor(desired_mass),requires_grad = False)
            #nn.Parameter(torch.tensor(self.unseenclass.size(0)/self.nclass),requires_grad = False)#  
        self.prob_prune = nn.Parameter(torch.tensor(prob_prune),requires_grad = False) 
         
        self.lambda_ = lambda_# coefficient to combine both loss functions
        ...
        if self.is_conv:
            print('is_conv = True: Learn CONV layer correct')
        
        if self.normalize_V:  
            print('normalize_V = True: normalize V')  
        else:  
            print('normalize_V = False: no constraint V')  
              
        if self.normalize_F:  
            print('normalize_F = True: normalize F')  
        else:  
            print('normalize_F = False: no constraint F')  
              
        if self.is_conservative:  
            print('is_conservative = True: training to exclude unseen class [seen upperbound]')  
        ...
        if self.non_linear_act:
            print('non_linear_act = True: Non-linear relu model')
        else:
            print('non_linear_act = False: Linear model')
        ...
        if self.uniform_att_1:
            print('uniform_att_1 = True: WARNING: UNIFORM ATTENTION LEVEL 1')
        if self.uniform_att_2:
            print('uniform_att_2 = True: WARNING: UNIFORM ATTENTION LEVEL 2')
        print('Compute Pruning loss {}'.format(self.prob_prune))  
        if self.is_bias:
            print('is_bias = True: Add one smoothing')
        print('Second layer attenion conditioned on image features')
        print('-'*30)  
        
        if self.non_linear_emb:
            print('non_linear_emb = True: non_linear embedding')
        ...
        if self.is_sigmoid:
            print("is_sigmoid = True: Sigmoid on attr score!!!")
        else:
            print("is_sigmoid = False: No sigmoid on attr score")
    ...
    def forward(self,Fs): 
        ...
        if self.is_conv:# is_conv=False, so we will ignore this
            Fs = self.conv1(Fs)
            Fs = self.conv1_bn(Fs)
            Fs = F.relu(Fs)
        
        shape = Fs.shape# 32, 7, 7, 2048
        Fs = Fs.reshape(shape[0],shape[1],shape[2]*shape[3])# shape = 32, 7, 14336
        
        R = Fs.size(2)  # number of rows  = 7
        B = Fs.size(0)  # batch size = 32
        V_n = self.compute_V()# normalize the w2v attribute semantic vectors
          
        if self.normalize_F and not self.is_conv:  
            Fs = F.normalize(Fs,dim = 1)  
          
        ## Compute attribute score on each image region
        S = torch.einsum('iv,vf,bfr->bir',V_n,self.W_1,Fs) # the confidence of having a-th attribute in the image i
        
        if self.is_sigmoid:
            S=torch.sigmoid(S)# apply sigmoid to S
