...
class DAZLE(nn.Module):  
    ...
    def __init__(self,dim_f,dim_v,  
        ...
        self.dim_f = dim_f  
        self.dim_v = dim_v  
        self.dim_att = att.shape[1]  #85
        self.nclass = att.shape[0]  # 50
        self.hidden = self.dim_att//2 # 85//2 = 42
        
        self.init_w2v_att = init_w2v_att # 85,300
        ...
        if init_w2v_att is None:  
            ...
            self.init_w2v_att = F.normalize(torch.tensor(init_w2v_att)) # 85,300
            self.V = nn.Parameter(self.init_w2v_att.clone(),requires_grad = trainable_w2v)  # 85,300 
        
        self.att = nn.Parameter(F.normalize(torch.tensor(att)),requires_grad = False) # 50,85         
        
        self.W_1 = nn.Parameter(nn.init.normal_(torch.empty(self.dim_v,self.dim_f)),requires_grad = True) # 300, 2048
        #nn.utils.weight_norm(nn.Linear(self.dim_v,self.dim_f,bias=False))#  
        self.W_2 = nn.Parameter(nn.init.zeros_(torch.empty(self.dim_v,self.dim_f)),requires_grad = True) # 300, 2048 #nn.utils.weight_norm(nn.Linear(self.dim_v,self.dim_f,bias=False))#  
        ## second layer attenion conditioned on image features
        self.W_3 = nn.Parameter(nn.init.zeros_(torch.empty(self.dim_v,self.dim_f)),requires_grad = True) # 300, 2048
        
        ## Compute the similarity between classes  
        self.P = torch.mm(self.att,torch.transpose(self.att,1,0))  # 50,50
        assert self.P.size(1)==self.P.size(0) and self.P.size(0)==self.nclass  # 50
        ...
        if self.non_linear_emb:
            ...
            self.emb_func = torch.nn.Sequential(
                                torch.nn.Linear(self.dim_att, self.dim_att//2), # 85, 42
                                torch.nn.ReLU(),
                                torch.nn.Linear(self.dim_att//2, 1), # 42, 1
                            )
    ...
    def forward(self,Fs): 
        ...
        R = Fs.size(2)  
        B = Fs.size(0)  # batch size
        V_n = self.compute_V() # 85, 300
          
        if self.normalize_F and not self.is_conv:  
            Fs = F.normalize(Fs,dim = 1)  
          
        ## Compute attribute score on each image region
        S = torch.einsum('iv,vf,bfr->bir',V_n, self.W_1, Fs) # 85,300   300,2048   
